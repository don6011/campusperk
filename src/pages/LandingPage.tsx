...
const WaitlistForm = ({ onSuccess, compact = false }: { onSuccess: (data: { email: string; campus: string; position: number }) => void; compact?: boolean }) => {
    ...
    try {
      const { error } = await supabase
        ...
        .insert({
          ...
          campus_name: selectedCampus || campusQuery || null,
          referral_source: hearAbout || null,
        } as any);
      ...
      // Get position
      const { data: posData } = await supabase.rpc("get_waitlist_position" as any, { p_email: email.toLowerCase().trim() });
      const position = (posData as number) ?? 0;

      onSuccess({ email, campus: selectedCampus || campusQuery, position });
    } catch {
      toast.error("Something went wrong. Please try again.");
    }
    setSubmitting(false);
  ...
  return (
    <form onSubmit={handleSubmit} className={`space-y-3 ${compact ? "max-w-md" : "max-w-lg"} mx-auto w-full`}>
      <Input
        type="email"
        required
      ...
      <div className="relative">
        ...
        <Input
          ...
          className="h-12 pl-10 bg-secondary border-border text-base"
        />
      ...
      {!compact && (
        <select
          ...
          className="flex h-12 w-full rounded-md border border-border bg-secondary px-3 py-2 text-base text-foreground ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
        >
          <option value="" className="text-muted-foreground">How did you hear about CampusPerk? (optional)</option>
...
const WaitlistSuccess = ({ data }: { data: { email: string; campus: string; position: number } }) => {
  const [copied, setCopied] = useState(false);
  const shareUrl = `${window.location.origin}?ref=waitlist`;
  const shareText = `I just joined CampusPerk — the student deal app with real discounts. Join early access: ${shareUrl}`;
    ...
    >
      ...
      {data.position > 0 && (
        <div className="mt-4 rounded-2xl border border-primary/30 bg-primary/5 p-6">
          <p className="text-sm text-muted-foreground mb-1">Your waitlist position</p>
          <div className="font-display text-5xl font-bold text-primary">#{data.position}</div>
          {data.campus && (
            <p className="mt-2 text-sm text-muted-foreground">
              on the waitlist for <span className="text-foreground font-semibold">{data.campus}</span>
            </p>
          )}
        </div>
      )}
      ...
      <p className="mt-4 text-muted-foreground text-sm">
        We'll notify you when CampusPerk launches on your campus.
      </p>

      <div className="mt-8 rounded-2xl border border-border bg-card p-6">
        <div className="flex items-center justify-center gap-2 mb-3">
          <Share2 className="h-4 w-4 text-primary" />
          <p className="text-sm font-semibold text-foreground">Move up the list by inviting friends</p>
        </div>
        <p className="text-xs text-muted-foreground mb-4">
          Invite friends from your campus to move up the list.
        </p>
        <div className="flex gap-3 justify-center">
          <a
            ...
            className="flex items-center gap-2 rounded-xl border border-border bg-secondary px-4 py-2.5 text-sm font-medium text-foreground hover:bg-secondary/80 transition-colors"
          >
...
const LandingPage = () => {
  // Campus search state (for campus search section)
  const [campusQuery, setCampusQuery] = useState("");
  ...
  // Waitlist state
  const [waitlistSuccess, setWaitlistSuccess] = useState<{ email: string; campus: string; position: number } | null>(null);
  ...
  return (
    <div className="min-h-screen bg-background overflow-hidden scroll-smooth">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-border/30 bg-background/70 backdrop-blur-xl">
        <div className="container mx-auto flex h-16 items-center justify-between px-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button size="sm" onClick={scrollToWaitlist} className="bg-accent hover:bg-accent/90 text-accent-foreground font-semibold">
              Join Early Access
            </Button>
          </div>
        </div>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <motion.div initial="hidden" animate="visible" className="max-w-xl">
              ...
              >
                The student deal app that only shows real discounts. Discover verified student deals from top brands and local businesses near your campus.
              </motion.p>
              ...
              >
                <Button size="lg" onClick={scrollToWaitlist} className="bg-accent hover:bg-accent/90 text-accent-foreground font-bold px-8 h-12 text-base gap-2 shadow-[0_0_30px_-5px_hsl(var(--accent)/0.4)] hover:shadow-[0_0_40px_-5px_hsl(var(--accent)/0.6)] transition-shadow">
                  Join Early Access
                </Button>
                <Link to="/partners/apply">
              ...
              >
                Launching soon for students nationwide.
              </motion.p>
      ...
      <section ref={waitlistRef} id="waitlist" className="py-20 md:py-28 border-t border-border/30">
        <div className="container mx-auto px-4">
          <AnimatePresence mode="wait">
            {waitlistSuccess ? (
              <WaitlistSuccess key="success" data={waitlistSuccess} />
            ) : (
              <motion.div
                key="form"
                initial="hidden"
              ...
              >
                ...
                <motion.p variants={fadeUp} custom={1} className="mx-auto mt-4 max-w-xl text-muted-foreground mb-10">
                ...
                <motion.p variants={fadeUp} custom={3} className="mt-4 text-xs text-muted-foreground">
                  Free early access. No spam. No account required.
                </motion.p>
              </motion.div>
            )}
          </AnimatePresence>
      ...
      <section className="py-20 md:py-28 border-t border-border/30">
        <div className="container mx-auto px-4">
          ...
          >
            <motion.h2 variants={fadeUp} custom={0} className="font-display text-3xl font-bold md:text-5xl">
              Why Join{" "}
              <span className="bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent">Early?</span>
            </motion.h2>
          </motion.div>
      ...
      <section id="deals" className="py-20 md:py-28 border-t border-border/30">
        <div className="container mx-auto px-4">
          <motion.div
            ...
            viewport={{ once: true, margin: "-100px" }}
          >
            <motion.p variants={fadeUp} custom={0} className="text-sm font-medium text-primary mb-2 uppercase tracking-wider">
              Trending on campuses right now
            </motion.p>
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4 max-w-5xl mx-auto">
            {trendingDeals.map((deal, i) => (
              ...
              >
                <div
                  className="mb-4 flex h-20 w-full items-center justify-center rounded-xl px-4"
                ...
                <div className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-accent/10 px-3 py-1 text-sm font-semibold text-accent">
                  {deal.savings}
                </div>
                <div className="mt-4">
                  <Button size="sm" onClick={scrollToWaitlist} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground gap-1 shadow-[0_0_20px_-5px_hsl(var(--primary)/0.3)] hover:shadow-[0_0_30px_-5px_hsl(var(--primary)/0.5)] transition-shadow">
          ...
          <motion.div
            ...
            className="mt-10 text-center"
          >
            <Button size="lg" onClick={scrollToWaitlist} className="bg-primary hover:bg-primary/90 text-primary-foreground gap-2">
        ...
        <div className="container mx-auto px-4">
          ...
          >
            ...
            <motion.p variants={fadeUp} custom={1} className="mt-4 text-muted-foreground">
              CampusPerk shows which schools are saving the most each week.
            </motion.p>
          </motion.div>
              ...
              {leaderboardData.map((entry, i) => (
                ...
                >
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center font-display font-bold text-lg shrink-0 ${
                    entry.rank === 1 ? "bg-[hsl(var(--gold))]/20 text-[hsl(var(--gold))]" :
                  ...
                  {entry.rank <= 3 && (
                    <Trophy className={`h-4 w-4 ${entry.rank === 1 ? "text-[hsl(var(--gold))]" : "text-primary/60"}`} />
                  )}
                </motion.div>
              ))}
            </div>
          ...
          >
            ...
            {matchedDealsCount !== null && !searchLoading && (
              ...
              >
                ...
                <Button onClick={scrollToWaitlist} className="mt-4 bg-accent hover:bg-accent/90 text-accent-foreground gap-2">
      ...
      <section id="categories" className="py-20 md:py-28 border-t border-border/30">
        <div className="container mx-auto px-4">
          ...
          >
            <motion.h2 variants={fadeUp} custom={0} className="font-display text-3xl font-bold md:text-5xl">
              10 Categories. Hundreds of Student Deals.
            </motion.h2>
            <motion.p variants={fadeUp} custom={1} className="mx-auto mt-4 max-w-xl text-muted-foreground">
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <motion.p variants={fadeUp} custom={1} className="mx-auto mt-4 max-w-lg text-muted-foreground">
                Join students discovering verified deals across campuses nationwide.
              </motion.p>
      ...
      <div className="fixed bottom-4 left-4 right-4 z-50 md:hidden">
        <Button onClick={scrollToWaitlist} className="w-full bg-accent hover:bg-accent/90 text-accent-foreground font-bold h-12 text-base gap-2 shadow-[0_0_30px_-5px_hsl(var(--accent)/0.5)] rounded-xl">
          Join Early Access <ArrowRight className="h-4 w-4" />
        </Button>
      </div>
    </div>
