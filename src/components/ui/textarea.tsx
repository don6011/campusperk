...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(({ className, ...props }, ref) => {
  return (
    <textarea
      className={cn(
        // 16px on mobile: iOS Safari zooms the viewport on focus for anything
        // smaller. `Input` already did this; this did not, so every textarea in
        // the product zoomed the page. 14px from md up, where no zoom applies.
