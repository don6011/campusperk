...
/**
 * One subscription per (type, category set). `categories` is an array, so an
 * empty one is a real value — every alert_type that ships without categories
 * collapses to a single "All deals" row rather than one row per insert.
 */
function alertKey(alert: Pick<AlertSubscription, "alert_type" | "categories">): string {
  const categories = [...(alert.categories ?? [])].sort().join(",");
  return `${alert.alert_type}::${categories}`;
}

function dedupeAlerts(rows: AlertSubscription[]): AlertSubscription[] {
...
export function AlertsSection() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      // Filter out the opt-out record, then collapse duplicates. Rows with no
      // category all render as "All deals", so seven of them looked like seven
      // separate subscriptions. The migration removes the existing duplicates;
      // this keeps the list honest against any row that predates it.
      setAlerts(dedupeAlerts(allAlerts.filter((a) => a.alert_type !== "leaderboard_optout")));
  ...
  const handleAdd = async () => {
    ...
    const existing = alerts.find(
      (a) => alertKey(a) === alertKey({ alert_type: "category", categories: [category.slug] }),
    );
    if (existing) {
    ...
    setAlerts((prev) => dedupeAlerts([data, ...prev]));
    setSelectedCategory("");
    ...
    <div className="space-y-6">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
          <h2 className="font-display text-xl font-bold text-foreground flex items-center gap-2">
            <Bell className="h-5 w-5 text-primary" /> Deal Alerts
          </h2>
          <p className="text-sm text-muted-foreground mt-1">
            Get notified when new deals drop in your favorite categories.
          </p>
          <a href="/notification-settings" className="inline-flex items-center gap-1.5 mt-2 text-xs text-brand hover:underline font-medium">
            ...
            <CardContent>
              ...
              {atLimit && !profile?.premium_status && (
                <p className="text-xs text-muted-foreground mt-2">
                  ...
                  <a href="/pricing" className="text-brand hover:underline font-medium">Upgrade to Premium</a> for unlimited.
                </p>
              )}
            </CardContent>
          </Card>
